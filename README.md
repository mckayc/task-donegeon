...
-   **Database Migration to SQLite & Drizzle:** The entire backend has been migrated from PostgreSQL to a more portable and lightweight SQLite database, managed by the modern Drizzle ORM. This simplifies setup, especially for local and Docker-based deployments, by removing the need for a separate database server.
...
-   **Backend:** Node.js with Express.js, **SQLite**, **Drizzle ORM**, Google Gemini
-   **Deployment:** Docker, Vercel (Frontend & Serverless Functions), Supabase (Storage Only)
...
2.  **Configure `.env` File:**
    Copy `.env.example` to `.env` in the project root and fill in the variables.
    -   `STORAGE_PROVIDER`: `local` (This is required for the server to start).
    -   **`API_KEY`** (Optional): Your Google Gemini API Key. Without this, AI features will be disabled.

3.  **Run Backend & Frontend:**
    -   In one terminal, from the `backend` directory: `npm start`
    -   In another terminal, from the root directory: `npm run dev`
    The application will be available at `http://localhost:5173` (or another port specified by Vite). The backend will create a `backend/db/app.db` file for the database.

### Option 2: Vercel & Supabase Deployment (Recommended for production)
1.  **Fork the Repository** to your own GitHub account.

2.  **Set up Supabase Project (for file storage only):**
...
3.  **Set up and Deploy on Vercel:**
    ...
    -   Click **Deploy**.
    -   **Note:** The server-side backup and local file storage features are designed for Docker/local deployments with persistent storage and will not work on Vercel's ephemeral filesystem. Only Supabase storage is supported.

### Option 3: Docker Deployment (The best self-hosted option)
...
2.  **Create local directories for persistent storage:**
    ...
    ```
3.  **Configure `.env` File:** Copy `docker.env.example` to `.env` and set the variables.
    -   `STORAGE_PROVIDER`: `local`
    -   **`API_KEY`** (Optional): Your Google Gemini API Key.
    -   **`APP_PORT`** (Optional): Change the port the app is exposed on. Defaults to `9373`.
    -   **`APP_DATA_PATH`** (Optional): Change the path on your host machine where data is stored. Defaults to `./data`.

4.  **Build and Run:**
    ```bash
    docker-compose up --build -d
    ```
    The app will be at `http://localhost:9373`. Database, uploaded files, and server-side backups will be persisted in the `./data` directory on your host machine.
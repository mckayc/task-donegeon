...
const { db } = require('./db');
const { appData } = require('./db/schema');
...
fs.mkdir(BACKUP_DIR, { recursive: true }).then(() => console.log(`Backup directory is ready at: ${BACKUP_DIR}`));
...
app.get('/api/data/load', async (req, res, next) => {
    try {
        const result = await db.select().from(appData).where(eq(appData.key, 'app_state'));
        const data = result.length > 0 ? JSON.parse(result[0].value) : {};
...
app.post('/api/data/save', async (req, res, next) => {
    try {
        ...
        await db.insert(appData)
          .values({ key: 'app_state', value: dataToSave })
          .onConflictDoUpdate({ 
            target: appData.key, 
            set: { value: dataToSave } 
          });
...
const runAutomatedBackup = async () => {
    try {
        ...
        const appState = JSON.parse(result[0].value);

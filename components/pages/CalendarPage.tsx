...
import { Quest, QuestType, QuestCompletion, QuestCompletionStatus, Role, QuestAvailability } from '../../types';
import Card from '../ui/Card';
import DailyDetailDialog from '../calendar/DailyDetailDialog';
...
const CalendarPage: React.FC = () => {
    ...
    const filteredQuests = useMemo(() => {
        ...
        return quests.filter(q => {
            // Only show active quests relevant to the user's scope and assignments
            if (!q.isActive || q.guildId !== currentGuildId) return false;
            if (q.assignedUserIds.length > 0) return q.assignedUserIds.includes(currentUser.id);
            return true;
        });
    ...
    const questsByDate = useMemo(() => {
        ...
        filteredQuests.forEach(quest => {
            if (quest.type === QuestType.Venture && quest.lateDateTime) {
                const dateKey = quest.lateDateTime.split('T')[0];
    ...
    const completionsByDate = useMemo(() => {
        ...
        relevantCompletions.forEach(completion => {
            const dateKey = completion.completedAt;
            const collection = map.get(dateKey) || [];
    ...
    const pendingCompletionsByDate = useMemo(() => {
        const currentGuildId = appMode.mode === 'guild' ? appMode.guildId : undefined;
        const relevantCompletions = questCompletions.filter(c =>
            c.status === QuestCompletionStatus.Pending &&
            c.userId === currentUser.id &&
            c.guildId === currentGuildId
        );
            ...
            <Card>
                <div className="flex items-center justify-between p-4 border-b border-stone-700/60 flex-wrap gap-4">
                    <div className="flex items-center">
                        <button onClick={() => changeMonth(-1)} className="p-2 rounded-full hover:bg-stone-700 transition">&lt;</button>
                        <h2 className="text-2xl font-semibold text-emerald-300 mx-4 text-center">{currentDate.toLocaleString('default', { month: 'long', year: 'numeric' })}</h2>
                        <button onClick={() => changeMonth(1)} className="p-2 rounded-full hover:bg-stone-700 transition">&gt;</button>
                    </div>
                </div>

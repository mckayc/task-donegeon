        ...
        <div className="space-y-8 relative">
            ...
            <CollapsibleSection title="AI Features">
             ...
             <CollapsibleSection title="Economy & Valuation" onSave={() => handleManualSave('Economy & Valuation')} showSavedIndicator={showSaved === 'Economy & Valuation'}>
                <div className="space-y-4">
                    ...
                    />
                    <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 mt-4 border-t border-border ${!formState.rewardValuation.enabled ? 'opacity-50 pointer-events-none' : ''}`}>
                        {/* Left Column: Anchor & Fees */}
                        <div className="space-y-4">
                             <div className="space-y-2">
                               <Label htmlFor="anchor-currency">Anchor Currency</Label>
                               <Select onValueChange={(value) => handleFormChange({ target: { name: 'rewardValuation.anchorRewardId', value } } as any)} defaultValue={formState.rewardValuation.anchorRewardId}>
                                 <SelectTrigger id="anchor-currency"><SelectValue /></SelectTrigger>
                                 <SelectContent>
                                   {currencyRewards.map(rt => <SelectItem key={rt.id} value={rt.id}>{rt.name}</SelectItem>)}
                                 </SelectContent>
                               </Select>
                             </div>
                             <div className="space-y-2">
                               <Label htmlFor="currency-fee">Currency Exchange Fee (%)</Label>
                               <Input id="currency-fee" type="number" step="0.1" name="rewardValuation.currencyExchangeFeePercent" value={formState.rewardValuation.currencyExchangeFeePercent} onChange={handleFormChange} />
                             </div>
                             <div className="space-y-2">
                               <Label htmlFor="xp-fee">XP Exchange Fee (%)</Label>
                               <Input id="xp-fee" type="number" step="0.1" name="rewardValuation.xpExchangeFeePercent" value={formState.rewardValuation.xpExchangeFeePercent} onChange={handleFormChange} />
                            </div>
                        </div>
                        {/* Right Column: Exchange Rates */}
                        <div className="space-y-3">
                            <h4 className="font-semibold text-foreground">Exchange Rates</h4>
                            <p className="text-sm text-muted-foreground -mt-2">How many of each reward is equal to 1 <span className="font-bold">{anchorReward?.name || 'Anchor'}</span>?</p>
                            <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                                {otherRewards.map(rt => (
                                    <div key={rt.id} className="flex items-center gap-2 p-2 bg-background rounded-md">
                                        <Label htmlFor={`exchange-${rt.id}`} className="flex-grow">{rt.name} ({rt.icon})</Label>
            ...
            <CollapsibleSection title={`Default ${formState.terminology.task} Values`} showSavedIndicator={showSaved === `Default ${formState.terminology.task} Values`}>
                 <div className="space-y-6">
                    <div className="flex items-start">
                        ...
                        <p className="text-sm ml-6 text-muted-foreground">If enabled, new {formState.terminology.tasks.toLowerCase()} will default to requiring approval from a {formState.terminology.moderator} or {formState.terminology.admin}.</p>
                    </div>
                    <div className="flex items-start">
                        ...
                        <p className="text-sm ml-6 text-muted-foreground">If enabled, new {formState.terminology.tasks.toLowerCase()} will default to being optional, meaning they don't count against a user if not completed.</p>
                    </div>
                    <div className="flex items-start">
                        ...
                        <p className="text-sm ml-6 text-muted-foreground">If enabled, new {formState.terminology.tasks.toLowerCase()} will be active and visible on the board immediately upon creation.</p>
                    </div>
                </div>
            </CollapsibleSection>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
                    {Object.keys(formState.terminology).map(key => (
                        <div key={key} className="space-y-2">
                         <Label htmlFor={`term-${key}`}>{terminologyLabels[key as keyof Terminology]}</Label>
                         <Input 
                            id={`term-${key}`}
            ...
            <CollapsibleSection title="Advanced">
                <div className="p-4 bg-destructive/10 border border-destructive/20 rounded-lg">
                    <h4 className="font-bold text-destructive">Reset All Settings</h4>

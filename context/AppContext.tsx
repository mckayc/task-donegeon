...
// --- STATE SLICES & CONTEXTS ---

// Auth State: Deals with users and login status
interface AuthState {
  users: User[];
...
// Game Data State: The core "database" of the app
...
// UI State: Transient state for the user interface
interface UIState {
  activePage: Page;
  appMode: AppMode;
...
// Settings State: App-wide configuration
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const stateRef = useRef<any | null>(null); // Ref for polling to prevent re-renders
    ...
    (async () => {
        ...
        if (!dataToSet) {
            ...
            dataToSet = {
                ...
                themes: INITIAL_THEMES, loginHistory: [], chatMessages: [],
            };
        }

        if (dataToSet) {
            const sampleMarkets = createSampleMarkets(); const sampleAssets = createSampleGameAssets();
            const requiredMarkets = sampleMarkets.filter(m => m.id !== 'market-tutorial');
            ...
            if (marketsToAdd.length > 0) { if (!dataToSet.markets) dataToSet.markets = []; dataToSet.markets.push(...marketsToAdd); }
            ...
            if (assetsToAdd.length > 0) { if (!dataToSet.gameAssets) dataToSet.gameAssets = []; dataToSet.gameAssets.push(...assetsToAdd); }

            const loadedSettings = {...INITIAL_SETTINGS, ...dataToSet.settings};
            setUsers(dataToSet.users || []); setQuests(dataToSet.quests || []); setMarkets(dataToSet.markets || []); setRewardTypes(dataToSet.rewardTypes || []);
            setQuestCompletions(dataToSet.questCompletions || []); setPurchaseRequests(dataToSet.purchaseRequests || []); setGuilds(dataToSet.guilds || []);
            setRanks(dataToSet.ranks || []); setTrophies(dataToSet.trophies || []); setUserTrophies(dataToSet.userTrophies || []);
            setAdminAdjustments(dataToSet.adminAdjustments || []); setGameAssets(dataToSet.gameAssets || []); setSystemLogs(dataToSet.systemLogs || []);
            setSettings(loadedSettings); setThemes(dataToSet.themes || INITIAL_THEMES); setLoginHistory(dataToSet.loginHistory || []);
            setChatMessages(dataToSet.chatMessages || []);

            const lastUserId = localStorage.getItem('lastUserId');
            if (lastUserId && dataToSet.users) { const lastUser = dataToSet.users.find((u:User) => u.id === lastUserId); if (lastUser) _setCurrentUser(lastUser); }
        ...
        try {
            ...
            if (aiResponse.ok) { setIsAiConfigured((await aiResponse.json()).isConfigured); }
        } catch (aiError) { console.error("Failed to fetch AI status:", aiError); setIsAiConfigured(false); }
  ...
  useEffect(() => {
    ...
    const saveData = async () => { try { await window.fetch('/api/data/save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(debouncedAppData) }); } catch (error) { console.error("Failed to save data:", error); } };
    saveData();
  }, [debouncedAppData, isDataLoaded, isRestoring]);
  ...
  useEffect(() => {
    ...
    const syncData = async () => { if (document.hidden) return; setSyncStatus('syncing'); setSyncError(null); try { const response = await window.fetch('/api/data/load'); if (!response.ok) { const errorText = await response.text(); throw new Error(`Server error: ${response.status} ${errorText || ''}`); } const serverData: IAppData = await response.json(); const currentLocalData = stateRef.current; if (currentLocalData && JSON.stringify(currentLocalData) !== JSON.stringify(serverData)) { console.log("Data out of sync. Refreshing from server."); const loadedSettings = {...INITIAL_SETTINGS, ...serverData.settings}; setUsers(serverData.users || []); setQuests(serverData.quests || []); setMarkets(serverData.markets || []); setRewardTypes(serverData.rewardTypes || []); setQuestCompletions(serverData.questCompletions || []); setPurchaseRequests(serverData.purchaseRequests || []); setGuilds(serverData.guilds || []); setRanks(serverData.ranks || []); setTrophies(serverData.trophies || []); setUserTrophies(serverData.userTrophies || []); setAdminAdjustments(serverData.adminAdjustments || []); setGameAssets(serverData.gameAssets || []); setSystemLogs(serverData.systemLogs || []); setSettings(loadedSettings); setThemes(serverData.themes || INITIAL_THEMES); setLoginHistory(serverData.loginHistory || []); setChatMessages(serverData.chatMessages || []); if (stateRef.current?.currentUser) { const updatedUser = (serverData.users || []).find(u => u.id === stateRef.current!.currentUser!.id); if (updatedUser) _setCurrentUser(updatedUser); } setSyncStatus('success'); } else { setSyncStatus('success'); } } catch (error) { console.error("Data sync failed:", error); setSyncStatus('error'); setSyncError(error instanceof Error ? error.message : 'An unknown error occurred during sync.'); } };
    const intervalId = setInterval(syncData, 15000); document.addEventListener('visibilitychange', syncData);
    return () => { clearInterval(intervalId); document.removeEventListener('visibilitychange', syncData); };
  ...
  const applyRewards = useCallback((userId: string, rewardsToApply: RewardItem[], guildId?: string) => { setUsers(prevUsers => { const userIndex = prevUsers.findIndex(u => u.id === userId); if (userIndex === -1) return prevUsers; const newUsers = [...prevUsers]; const userToUpdate = { ...newUsers[userIndex] }; rewardsToApply.forEach(reward => { const rewardDef = rewardTypes.find(rd => rd.id === reward.rewardTypeId); if (!rewardDef) return; if (guildId) { userToUpdate.guildBalances = { ...userToUpdate.guildBalances }; if (!userToUpdate.guildBalances[guildId]) userToUpdate.guildBalances[guildId] = { purse: {}, experience: {} }; const balanceSheet = { ...userToUpdate.guildBalances[guildId] }; if (rewardDef.category === RewardCategory.Currency) { balanceSheet.purse = { ...balanceSheet.purse }; balanceSheet.purse[reward.rewardTypeId] = (balanceSheet.purse[reward.rewardTypeId] || 0) + reward.amount; } else { balanceSheet.experience = { ...balanceSheet.experience }; balanceSheet.experience[reward.rewardTypeId] = (balanceSheet.experience[reward.rewardTypeId] || 0) + reward.amount; } userToUpdate.guildBalances[guildId] = balanceSheet; } else { if (rewardDef.category === RewardCategory.Currency) { userToUpdate.personalPurse = { ...userToUpdate.personalPurse }; userToUpdate.personalPurse[reward.rewardTypeId] = (userToUpdate.personalPurse[reward.rewardTypeId] || 0) + reward.amount; } else { userToUpdate.personalExperience = { ...userToUpdate.personalExperience }; userToUpdate.personalExperience[reward.rewardTypeId] = (userToUpdate.personalExperience[reward.rewardTypeId] || 0) + reward.amount; } } }); newUsers[userIndex] = userToUpdate; if (currentUser?.id === userId) { _setCurrentUser(userToUpdate); } return newUsers; }); }, [rewardTypes, currentUser]);
  const checkAndAwardTrophies = useCallback((userId: string, guildId?: string) => { if (guildId) return; const user = users.find(u => u.id === userId); if (!user) return; const userCompletedQuests = questCompletions.filter(c => c.userId === userId && !c.guildId && c.status === QuestCompletionStatus.Approved); const totalXp = Object.values(user.personalExperience).reduce((sum, amount) => sum + amount, 0); const userRank = ranks.slice().sort((a, b) => b.xpThreshold - a.xpThreshold).find(r => totalXp >= r.xpThreshold); const automaticTrophies = trophies.filter(t => !t.isManual); for (const trophy of automaticTrophies) { if (userTrophies.some(ut => ut.userId === userId && ut.trophyId === trophy.id && !ut.guildId)) continue; const meetsAllRequirements = trophy.requirements.every(req => { switch (req.type) { case TrophyRequirementType.CompleteQuestType: return userCompletedQuests.filter(c => quests.find(q => q.id === c.questId)?.type === req.value).length >= req.count; case TrophyRequirementType.CompleteQuestTag: return userCompletedQuests.filter(c => quests.find(q => q.id === c.questId)?.tags?.includes(req.value)).length >= req.count; case TrophyRequirementType.AchieveRank: return userRank?.id === req.value; default: return false; } }); if (meetsAllRequirements) { setUserTrophies(prev => [...prev, { id: `award-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`, userId, trophyId: trophy.id, awardedAt: new Date().toISOString(), guildId }]); addNotification({ type: 'trophy', message: `Trophy Unlocked: ${trophy.name}!`, icon: trophy.icon }); } } }, [users, questCompletions, ranks, trophies, userTrophies, quests, addNotification]);
  ...
  const addUser = useCallback((userData: Omit<User, 'id' | 'personalPurse' | 'personalExperience' | 'guildBalances' | 'avatar' | 'ownedAssetIds' | 'ownedThemes' | 'hasBeenOnboarded'>): User => { const newUser: User = { ...userData, id: `user-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`, avatar: {}, ownedAssetIds: [], personalPurse: {}, personalExperience: {}, guildBalances: {}, ownedThemes: ['emerald', 'rose', 'sky'], hasBeenOnboarded: false }; setUsers(prev => [...prev, newUser]); setGuilds(prev => prev.map(g => g.isDefault ? { ...g, memberIds: [...g.memberIds, newUser.id] } : g)); const roleTag = `tutorial-${newUser.role.toLowerCase().replace(/ /g, '-')}`; setQuests(prevQuests => prevQuests.map(q => { if (q.tags.includes(roleTag)) { if (!q.assignedUserIds.includes(newUser.id)) return { ...q, assignedUserIds: [...q.assignedUserIds, newUser.id] }; } return q; })); return newUser; }, []);
  const updateUser = useCallback((userId: string, updatedData: Partial<User>) => { setUsers(prev => prev.map(u => u.id === userId ? { ...u, ...updatedData } : u)); if (currentUser?.id === userId) { _setCurrentUser(prev => prev ? { ...prev, ...updatedData } as User : null); } }, [currentUser]);
  ...
  const completeQuest = useCallback((questId: string, userId: string, rewards: RewardItem[], requiresApproval: boolean, guildId?: string, options?: { note?: string; completionDate?: Date }) => { const newCompletion: QuestCompletion = { id: `comp-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`, questId, userId, completedAt: (options?.completionDate || new Date()).toISOString(), status: requiresApproval ? QuestCompletionStatus.Pending : QuestCompletionStatus.Approved, guildId, note: options?.note }; setQuestCompletions(prev => [...prev, newCompletion]); if (!requiresApproval) { applyRewards(userId, rewards, guildId); const quest = quests.find(q => q.id === questId); addNotification({ type: 'success', message: `Quest Completed: ${quest?.title}`}); checkAndAwardTrophies(userId, guildId); } else { addNotification({ type: 'info', message: `Quest submitted for approval.` }); } }, [quests, applyRewards, addNotification, checkAndAwardTrophies]);
  ...
  const deductRewards = useCallback((userId: string, cost: RewardItem[], guildId?: string): boolean => { const user = users.find(u => u.id === userId); if (!user) return false; const canAfford = cost.every(item => { const rd = rewardTypes.find(rt => rt.id === item.rewardTypeId); if (!rd) return false; const bal = guildId ? (rd.category === 'Currency' ? user.guildBalances[guildId]?.purse[item.rewardTypeId] : user.guildBalances[guildId]?.experience[item.rewardTypeId]) : (rd.category === 'Currency' ? user.personalPurse[item.rewardTypeId] : user.personalExperience[item.rewardTypeId]); return (bal || 0) >= item.amount; }); if (!canAfford) return false; const updatedUser = { ...user }; cost.forEach(c => { const rd = rewardTypes.find(rt => rt.id === c.rewardTypeId); if (!rd) return; if (guildId) { if (!updatedUser.guildBalances[guildId]) updatedUser.guildBalances[guildId] = { purse: {}, experience: {} }; if (rd.category === 'Currency') { updatedUser.guildBalances[guildId].purse[c.rewardTypeId] = (updatedUser.guildBalances[guildId].purse[c.rewardTypeId] || 0) - c.amount; } else { updatedUser.guildBalances[guildId].experience[c.rewardTypeId] = (updatedUser.guildBalances[guildId].experience[c.rewardTypeId] || 0) - c.amount; } } else { if (rd.category === 'Currency') { updatedUser.personalPurse[c.rewardTypeId] = (updatedUser.personalPurse[c.rewardTypeId] || 0) - c.amount; } else { updatedUser.personalExperience[c.rewardTypeId] = (updatedUser.personalExperience[c.rewardTypeId] || 0) - c.amount; } } }); setUsers(prevUsers => prevUsers.map(u => u.id === userId ? updatedUser : u)); if (currentUser?.id === userId) { _setCurrentUser(updatedUser); } return true; }, [users, rewardTypes, currentUser]);
  const purchaseMarketItem = useCallback((assetId: string, marketId: string, user: User) => { const market = markets.find(m => m.id === marketId); const asset = gameAssets.find(ga => ga.id === assetId); if (!market || !asset) return; if (deductRewards(user.id, asset.cost, market.guildId)) { let wasModified = false; const updatedUser = { ...user }; if (asset.payouts && asset.payouts.length > 0) { applyRewards(user.id, asset.payouts, market.guildId); addNotification({ type: 'success', message: `Exchanged for ${asset.name}!` }); wasModified = true; } if (asset.linkedThemeId && !updatedUser.ownedThemes.includes(asset.linkedThemeId)) { updatedUser.ownedThemes = [...updatedUser.ownedThemes, asset.linkedThemeId]; addNotification({ type: 'success', message: `Theme Unlocked: ${asset.name}!` }); wasModified = true; } if (!asset.payouts || asset.payouts.length === 0) { updatedUser.ownedAssetIds = [...updatedUser.ownedAssetIds, asset.id]; addNotification({ type: 'success', message: `Purchased "${asset.name}"!` }); wasModified = true; } if (wasModified) { updateUser(user.id, updatedUser); } setPurchaseRequests(p => [...p, { id: `purchase-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`, userId: user.id, assetId, requestedAt: new Date().toISOString(), status: PurchaseRequestStatus.Completed, assetDetails: { name: asset.name, description: asset.description, cost: asset.cost }, guildId: market.guildId }]); } else { addNotification({ type: 'error', message: 'You cannot afford this item.' }); } }, [markets, gameAssets, deductRewards, addNotification, applyRewards, updateUser]);
  ...
  const populateInitialGameData = useCallback((adminUser: User) => { addNotification({ type: 'info', message: 'Your Donegeon is being populated!' }); const sA = createMockUsers().filter(u => u.role !== Role.DonegeonMaster); const aIU = [adminUser, ...sA]; setUsers(aIU); const newQuests = createSampleQuests(aIU); setQuests(newQuests); setMarkets(createSampleMarkets()); setGameAssets(createSampleGameAssets()); setRewardTypes(INITIAL_REWARD_TYPES); setGuilds(createInitialGuilds(aIU)); setRanks(INITIAL_RANKS); setTrophies(INITIAL_TROPHIES); setQuestCompletions(createInitialQuestCompletions(aIU, newQuests)); }, [addNotification]);
  const restoreFromBackup = useCallback(async (backupData: IAppData) => { setIsRestoring(true); try { await window.fetch('/api/data/save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(backupData) }); addNotification({ type: 'success', message: 'Restore successful! The app will now reload.' }); setTimeout(() => window.location.reload(), 1500); } catch (e) { addNotification({ type: 'error', message: 'Failed to restore from backup.' }); setIsRestoring(false); } }, [addNotification]);
  ...
  const addTheme = useCallback((theme: Omit<ThemeDefinition, 'id'>) => { const newTheme: ThemeDefinition = { ...theme, id: `custom-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`, isCustom: true }; setThemes(prev => [...prev, newTheme]); addNotification({ type: 'success', message: 'Theme created!' }); }, [addNotification]);
  const updateTheme = useCallback((theme: ThemeDefinition) => { setThemes(prev => prev.map(t => t.id === theme.id ? theme : t)); addNotification({ type: 'success', message: 'Theme updated!' }); }, [addNotification]);
  const deleteTheme = useCallback((themeId: string) => { setThemes(prev => prev.filter(t => t.id !== themeId)); addNotification({ type: 'info', message: 'Theme deleted.' }); }, [addNotification]);
  ...
  const updateSettings = useCallback((settingsToUpdate: Partial<AppSettings>) => { setSettings(prev => ({...prev, ...settingsToUpdate})); }, []);
  ...
  useEffect(() => { if (!settings.automatedBackups.enabled || !isDataLoaded) return; const performBackup = () => { try { const savedBackupsRaw = localStorage.getItem('localBackups'); const savedBackups = savedBackupsRaw ? JSON.parse(savedBackupsRaw) : []; let autoBackups = savedBackups.filter((b: any) => b.isAuto); const dataToBackup = { users, quests, markets, rewardTypes, questCompletions, purchaseRequests, guilds, ranks, trophies, userTrophies, adminAdjustments, gameAssets, systemLogs, settings, themes, loginHistory, chatMessages }; const dataStr = JSON.stringify(dataToBackup); const size = new Blob([dataStr]).size; const newBackup = { id: `auto-backup-${Date.now()}`, timestamp: new Date().toISOString(), name: `auto_backup_${new Date().toISOString().replace(/:/g, '-').slice(0, 19)}.json`, data: dataToBackup, size: size, isAuto: true, }; autoBackups.unshift(newBackup); if (autoBackups.length > settings.automatedBackups.maxBackups) { autoBackups = autoBackups.slice(0, settings.automatedBackups.maxBackups); } const manualBackups = savedBackups.filter((b: any) => !b.isAuto); localStorage.setItem('localBackups', JSON.stringify([...manualBackups, ...autoBackups])); console.log(`Automated backup created at ${new Date().toLocaleString()}`); } catch (e) { console.error("Failed to perform automated backup:", e); } }; const intervalId = setInterval(performBackup, settings.automatedBackups.frequencyHours * 60 * 60 * 1000); return () => clearInterval(intervalId); }, [settings.automatedBackups, appData, isDataLoaded]);
  const toggleChat = useCallback(() => setIsChatOpen(prev => !prev), []);
  const sendMessage = useCallback((message: Omit<ChatMessage, 'id' | 'timestamp' | 'isRead' | 'senderId'>) => { if (!currentUser) return; const newMessage: ChatMessage = { id: `msg-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`, senderId: currentUser.id, timestamp: new Date().toISOString(), isRead: false, ...message }; setChatMessages(prevMessages => { const newMessages = [...prevMessages, newMessage]; const currentState = stateRef.current; if (currentState) { const updatedStateForSave = { ...currentState, chatMessages: newMessages }; stateRef.current = updatedStateForSave; window.fetch('/api/data/save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updatedStateForSave) }).catch(error => { console.error("Failed to save chat message immediately:", error); }); } return newMessages; }); }, [currentUser]);
  const markMessagesAsRead = useCallback((senderId: string) => { if (!currentUser) return; setChatMessages(prev => prev.map(msg => (msg.senderId === senderId && msg.recipientId === currentUser.id && !msg.isRead) ? { ...msg, isRead: true } : msg )); }, [currentUser]);

  // === CONTEXT PROVIDER VALUES ===
  const authState: AuthState = useMemo(() => ({ users, currentUser, isAppUnlocked, isFirstRun, isSwitchingUser, targetedUserForLogin, loginHistory }), [users, currentUser, isAppUnlocked, isFirstRun, isSwitchingUser, targetedUserForLogin, loginHistory]);
  const gameDataState: GameDataState = useMemo(() => ({ quests, markets, rewardTypes, questCompletions, purchaseRequests, guilds, ranks, trophies, userTrophies, adminAdjustments, gameAssets, systemLogs, themes, allTags: Array.from(new Set(quests.flatMap(q => q.tags || []))).sort() }), [quests, markets, rewardTypes, questCompletions, purchaseRequests, guilds, ranks, trophies, userTrophies, adminAdjustments, gameAssets, systemLogs, themes]);
  const uiState: UIState = useMemo(() => ({ activePage, appMode, notifications, isDataLoaded, activeMarketId, isSharedViewActive, isSidebarCollapsed, syncStatus, syncError, isChatOpen, chatMessages }), [activePage, appMode, notifications, isDataLoaded, activeMarketId, isSharedViewActive, isSidebarCollapsed, syncStatus, syncError, isChatOpen, chatMessages]);
  const settingsState: SettingsState = useMemo(() => ({ settings, isAiConfigured }), [settings, isAiConfigured]);

  stateRef.current = { users, quests, markets, rewardTypes, questCompletions, purchaseRequests, guilds, ranks, trophies, userTrophies, adminAdjustments, gameAssets, systemLogs, settings, themes, loginHistory, chatMessages, currentUser };
  const dispatchValue: AppDispatch = { addUser, updateUser, deleteUser, setCurrentUser, markUserAsOnboarded, setAppUnlocked, setIsSwitchingUser, setTargetedUserForLogin, exitToSharedView, setIsSharedViewActive: _setIsSharedViewActive, addQuest, updateQuest, deleteQuest, dismissQuest, claimQuest, releaseQuest, markQuestAsTodo, unmarkQuestAsTodo, completeQuest, approveQuestCompletion, rejectQuestCompletion, addRewardType, updateRewardType, deleteRewardType, addMarket, updateMarket, deleteMarket, purchaseMarketItem, cancelPurchaseRequest, approvePurchaseRequest, rejectPurchaseRequest, addGuild, updateGuild, deleteGuild, setRanks, addTrophy, updateTrophy, deleteTrophy, awardTrophy, applyManualAdjustment, addGameAsset, updateGameAsset, deleteGameAsset, addTheme, updateTheme, deleteTheme, populateInitialGameData, importBlueprint, restoreFromBackup, clearAllHistory, resetAllPlayerData, deleteAllCustomContent, deleteSelectedAssets, deleteQuests, deleteTrophies, deleteGameAssets, updateQuestsStatus, uploadFile, updateSettings, setActivePage, setAppMode, addNotification, removeNotification, setActiveMarketId, toggleSidebar, toggleChat, sendMessage, markMessagesAsRead };

  return (
    <AuthStateContext.Provider value={authState}>
      <GameDataStateContext.Provider value={gameDataState}>
...
export const useAuthState = (): AuthState => { const context = useContext(AuthStateContext); if (context === undefined) throw new Error('useAuthState must be used within an AppProvider'); return context; };
export const useGameDataState = (): GameDataState => { const context = useContext(GameDataStateContext); if (context === undefined) throw new Error('useGameDataState must be used within an AppProvider'); return context; };
export const useUIState = (): UIState => { const context = useContext(UIStateContext); if (context === undefined) throw new Error('useUIState must be used within an AppProvider'); return context; };
export const useSettingsState = (): SettingsState => { const context = useContext(SettingsStateContext); if (context === undefined) throw new Error('useSettingsState must be used within an AppProvider'); return context; };
export const useAppDispatch = (): AppDispatch => { const context = useContext(AppDispatchContext); if (context === undefined) throw new Error('useAppDispatch must be used within an AppProvider'); return context; };

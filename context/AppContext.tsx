...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const refreshDataFromServer = useCallback(async (source: 'poll' | 'websocket' | 'visibility') => {
    ...
    try {
        ...
        // Merge optimistic chat messages if they exist locally but not on the server.
        let finalChatMessages = serverData.chatMessages || [];
        if (stateRef.current?.chatMessages) {
  ...
  const deductRewards = useCallback((userId: string, cost: RewardItem[], guildId?: string): boolean => {
    ...
    // A deep copy is crucial here to avoid mutating the original state object directly.
  ...
  const applyManualAdjustment = useCallback((adj: Omit<AdminAdjustment, 'id' | 'adjustedAt'>): boolean => {
    ...
    let success = false;
    if (adj.type === AdminAdjustmentType.Reward) {
        applyRewards(adj.userId, adj.rewards, adj.guildId);
        success = true;
    } else if (adj.type === AdminAdjustmentType.Setback) {
        success = deductRewards(adj.userId, adj.setbacks, adj.guildId);
        if (!success) {
            addNotification({ type: 'error', message: 'Setback failed. User does not have sufficient funds.' });
        }
    } else if (adj.type === AdminAdjustmentType.Trophy && adj.trophyId) {

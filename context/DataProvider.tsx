...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const syncData = useCallback(async () => {
    ...
    try {
        ...
        console.log('[DataProvider] Received sync updates:', JSON.stringify(updates, null, 2));
        ...
        try {
            ...
            console.error('[DataProvider] CRITICAL ERROR during state update dispatch:', error);
            addNotification({ type: 'error', message: `A critical error occurred while updating data: ${error instanceof Error ? error.message : 'Unknown error'}. Check the console for details.`, duration: 0 });
        }
  ...
  }, [users, setUsers, setCurrentUser, setLoginHistory, updateUser, questsDispatch, economyDispatch, progressionDispatch, communityDispatch, systemDispatch, addNotification]);

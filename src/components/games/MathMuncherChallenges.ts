...
const safeEval = (expr: string): number | null => {
    try {
        // A very basic santizer to prevent anything but simple math.
        if (/[^0-9+\-*/.() ]/.test(expr)) {
            return null;
        }
...
const grade2: MathChallenge[] = [
    { title: 'Munch sums that equal 15', gridSize: 6, generateGrid: () => generateChallengeGrid(() => { const a = getRandomInt(1,14); return `${a}+${15-a}`; }, () => { const a = getRandomInt(1,20); const b = getRandomInt(1,20); return a+b === 15 ? `${a}+${b+1}` : `${a}+${b}`;}, (v) => safeEval(String(v)) === 15)},
    { title: 'Munch differences that equal 11', gridSize: 6, generateGrid: () => generateChallengeGrid(() => { const a = getRandomInt(11,20); return `${a}-${a-11}`; }, () => {const a=getRandomInt(1,20); const b=getRandomInt(1,20); return a-b === 11 ? `${a}-${b+1}` : `${a}-${b}`;}, (v) => safeEval(String(v)) === 11)},
    { title: 'Munch numbers when skip-counting by 3s', gridSize: 6, generateGrid: () => generateChallengeGrid(() => getRandomInt(1,10)*3, () => { let n; do { n = getRandomInt(1,30); } while (n % 3 === 0); return n; }, (n) => typeof n === 'number' && n > 0 && n % 3 === 0) },
    { title: 'Munch facts that equal 20', gridSize: 6, generateGrid: () => {
        const correctOptions = ['2×10', '10×2', '4×5', '5×4'];
        return generateChallengeGrid(() => correctOptions[getRandomInt(0,3)], () => `${getRandomInt(2,10)}×${getRandomInt(2,10)}`, v => safeEval(String(v).replace('×','*')) === 20)
    }},
    { title: 'Munch facts that equal 2', gridSize: 6, generateGrid: () => {
        const correctOptions = ['10÷5', '20÷10', '4÷2', '8÷4', '6÷3'];
        return generateChallengeGrid(() => correctOptions[getRandomInt(0,4)], () => `${getRandomInt(5,20)}÷${getRandomInt(3,10)}`, v => safeEval(String(v).replace('÷','/')) === 2)
    }},
    ...
    { title: 'Munch numbers with 4 in the tens place', gridSize: 6, generateGrid: () => generateChallengeGrid(() => 40+getRandomInt(0,9), () => { let n; do {n = getRandomInt(10,99);} while(Math.floor(n/10) % 10 === 4); return n;}, (n) => typeof n === 'number' && Math.floor(n/10) % 10 === 4)},
...
const grade3: MathChallenge[] = [
    { title: 'Munch facts that equal 24', gridSize: 6, generateGrid: () => {
        ...
        return generateChallengeGrid(()=>correct[getRandomInt(0,5)], ()=>`${getRandomInt(2,12)}×${getRandomInt(2,12)}`, v=>safeEval(String(v).replace('×','*'))===24)
    }},
    { title: 'Munch facts that equal 7', gridSize: 6, generateGrid: () => {
        const correct = ['14÷2', '21÷3', '28÷4', '35÷5', '42÷6', '49÷7'];
        return generateChallengeGrid(()=>correct[getRandomInt(0,5)], ()=>`${getRandomInt(10,50)}÷${getRandomInt(2,7)}`, v=>safeEval(String(v).replace('÷','/'))===7)
    }},
    ...
    { title: 'Munch answers that equal 72', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>(`${getRandomInt(1,71)}+${72-getRandomInt(1,71)}`), ()=>(`${getRandomInt(20,50)}+${getRandomInt(20,50)}`), v=>safeEval(String(v))===72)},
    { title: 'Munch equations equal to 3×4', gridSize: 6, generateGrid: () => {
        ...
        const checker = (v: CellValue) => { try { return safeEval(String(v).replace('×','*')) === 12; } catch(e) { return false; } };
    ...
    { title: 'Munch answers to: How many 5s in 25?', gridSize: 6, generateGrid: () => generateChallengeGrid(() => ['5', '25÷5'][getRandomInt(0,1)], () => [getRandomInt(1,4), getRandomInt(6,10)][getRandomInt(0,1)], (v) => safeEval(String(v).replace('÷','/')) === 5) },
];

// --- GRADE 4 ---
const grade4: MathChallenge[] = [
    { title: 'Munch answers equal to 92', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>['23×4','46×2'][getRandomInt(0,1)], ()=>`${getRandomInt(20,50)}×${getRandomInt(2,4)}`, v=>safeEval(String(v).replace('×','*'))===92)},
...
const grade5: MathChallenge[] = [
    { title: 'Munch products equal to 1,764', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>'42×42', ()=>'30×60', v=>safeEval(String(v).replace('×','*')) === 1764)},
    { title: 'Munch facts equal to 24', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>'72÷3', ()=>'65÷2', v=>safeEval(String(v).replace('÷','/')) === 24)},
    { title: 'Munch fractions equal to 7/8', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>'3/8+4/8', ()=>'2/8+2/8', v=>safeEval(String(v)) === 7/8)},
    { title: 'Munch fractions equal to 2/3', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>'4/6', ()=>'8/12', v=>safeEval(String(v)) === 2/3)},
    { title: 'Munch fractions equal to 2 1/2', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>'5/2', ()=>'9/5', v=>safeEval(String(v)) === 2.5)},
    { title: 'Munch sums equal to 1.25', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>'0.75+0.50', ()=>'0.60+0.70', v=>safeEval(String(v)) === 1.25)},
    { title: 'Munch numbers equal to 25% of 100', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>[25, '1/4'][getRandomInt(0,1)], ()=>[50,75][getRandomInt(0,1)], v=>safeEval(String(v)) === 25)},
    { title: 'Munch equations equal to 14', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>'(2+5)×2', ()=>'3+4×2', v=>safeEval(String(v).replace('×','*'))===14)},
...
const grade6: MathChallenge[] = [
    { title: 'Munch products equal to 3/8', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>'1/2×3/4', ()=>'2/3×1/4', v=>safeEval(String(v).replace('×','*'))===3/8)},
    { title: 'Munch answers equal to 6', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>'3÷(1/2)', ()=>'5÷(1/3)', v=>safeEval(String(v).replace('÷','/'))===6)},
    { title: 'Munch sums equal to 1', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>'1/2+1/2', ()=>'3/4+1/3', v=>safeEval(String(v))===1)},
    { title: 'Munch values equal to 0.75', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>['3/4', '75%'][getRandomInt(0,1)], ()=>'2/5', v=>safeEval(String(v).replace('%','/100'))===0.75)},
    ...
    { title: 'Munch answers equal to 20% of 60', gridSize: 6, generateGrid: () => generateChallengeGrid(()=>[12, '0.2×60'][getRandomInt(0,1)], ()=>15, v=>safeEval(String(v).replace('×','*'))===12)},

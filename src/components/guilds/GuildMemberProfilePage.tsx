...
const GuildMemberProfilePage: React.FC<GuildMemberProfilePageProps> = ({ user, guild, onBack }) => {
    ...
    const rankData = useMemo(() => {
        ...
        const totalXp = Object.values(personalBalances.experience).reduce<number>((sum, amount) => sum + Number(amount), 0);
    ...
    const personalCurrencies: (RewardTypeDefinition & {amount: number})[] = useMemo(() => {
        return rewardTypes
    ...
    return (
        <div>
            ...
            <Card>
                <div className="flex flex-col sm:flex-row items-center gap-6 p-6 border-b border-stone-700/60">
                    <Avatar user={user} className="w-24 h-24 rounded-full overflow-hidden bg-stone-700 border-4 border-accent" />
                    <div className="text-center sm:text-left">
                        <h2 className="text-3xl font-bold text-stone-100">{user.gameName}</h2>
                        <p className="text-stone-400">{user.role}</p>
                        {rankData.currentRank && (
                             <div className="flex items-center gap-2 mt-2 justify-center sm:justify-start">
                                ...
                                <span className="text-sm text-stone-400">({rankData.totalXp} XP)</span>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className="p-6 border-t border-stone-700/60">
                    <h3 className="text-xl font-bold text-stone-200 mb-4">Item Collection</h3>
                     ...
                     ) : <p className="text-stone-400 text-sm italic">No items collected.</p>}
                </div>
            </Card>
        </div>
    );
